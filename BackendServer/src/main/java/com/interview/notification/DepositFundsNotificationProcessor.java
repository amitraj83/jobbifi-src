...
public class DepositFundsNotificationProcessor extends NotificationProcessor {
   ...
   * basicDBObject = (BasicDBObject) row.get(VARIABLES.NOTIFICATION.DATASTORE.CONTENT); ObjectId tid
   * = new ObjectId( basicDBObject.get(DATASTORES.TRANSACTION.TID).toString()); Transaction
   * transaction = Services.getInstance().getTransactionStore().getTransaction(tid);
   * //contentMap.put(DATASTORES.TRANSACTION.OWNER, transaction.getOwner());
   * contentMap.put(DATASTORES.TRANSACTION.NETAMOUNT, transaction.getNetamount()); } catch
  ...
  public void process(Object document) {
    try {
      ...
      cMap.put(DATASTORES.TRANSACTION.TID, transaction.getId());
      notification.setContent(cMap);
      notification.setCreatedBy(transaction.getOwner());
