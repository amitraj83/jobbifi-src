...
public class NewBidNotificationProcessor extends NotificationProcessor {
  ...
  public void process(Object document) {
    try {
      ...
      nData.put(VARIABLES.BID_ID, bid.getId());
      nData.put(VARIABLES.IID, bid.getIid());

      notification.setContent(nData);
      notification.setCreatedBy(bid.getBidder());
   ...
   * basicDBObject.get(VARIABLES.IID).toString()); ObjectId bidid = new ObjectId(
   * basicDBObject.get(VARIABLES.BID_ID).toString()); Interview interview =
   * Services.getInstance().getInterviewDataStore().getInterview(iid); Bid bid =
   * Services.getInstance().getBidStore().getBid(bidid);
   * 
   * contentMap.put(DATASTORES.INTERVIEW.TITLE, interview.getTitle()); contentMap.put(VARIABLES.IID,
   * interview.getId()); contentMap.put(VARIABLES.BID_ID, bidid.toString());
   * contentMap.put(VARIABLES.Bid.BIDDER, bid.getBidder()); contentMap.put(VARIABLES.Bid.DATE,
   * bid.getDate()); contentMap.put(VARIABLES.Bid.PRICE, bid.getPrice()); } catch (RemoteException
   * e) { // TODO Auto-generated catch block e.printStackTrace(); } return contentMap; }
   */
