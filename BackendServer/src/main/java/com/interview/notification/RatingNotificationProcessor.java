...
public class RatingNotificationProcessor extends NotificationProcessor {
  ...
  /*
   * @Override public Map<Object, Object> createContentMap(DBObject row) {
   * 
   * Map<Object, Object> contentMap = new HashMap<Object, Object>(); try{ BasicDBObject
   * basicDBObject = (BasicDBObject) row.get(VARIABLES.NOTIFICATION.DATASTORE.CONTENT); ObjectId rid
   * = new ObjectId( basicDBObject.get(DATASTORES.RATING.ID).toString());
   * 
   * Rating rating = Services.getInstance().getRatingStore().getRating(rid);
   * 
   * Interview interview = Services.getInstance().getInterviewDataStore().getInterview
   * (rating.getIid());
   * 
   * contentMap.put(DATASTORES.RATING.USERNAME, rating.getUsername());
   * contentMap.put(DATASTORES.RATING.AVERAGE, rating.getAverage()); contentMap.put("title",
   * interview.getTitle()); } catch (RemoteException e) { // TODO Auto-generated catch block
   * e.printStackTrace(); } return contentMap; }
   */
  @Override
  public void process(Object document) {
    try {
      Rating rating = (Rating) document;
      Notification notification = new Notification();

      Map<Object, Object> nMap = new HashMap<Object, Object>();
      nMap.put(DATASTORES.RATING.ID, rating.getId());
      notification.setContent(nMap);
      notification.setCreatedBy(rating.getRatedBy());
