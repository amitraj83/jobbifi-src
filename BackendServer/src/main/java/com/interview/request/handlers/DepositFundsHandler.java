...
public class DepositFundsHandler extends RequestHandler {
  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    if (null != subReq && REQUEST_TYPES.DEPOSIT_FUNDS_SUB_REQ.LOG_TRANSACTION.equals(subReq)) {
      try {
        ...
        && REQUEST_TYPES.DEPOSIT_FUNDS_SUB_REQ.CANCEL_TRANSACTION.equals(subReq)) {
      Services.getInstance().getFundsDepositService().updateTransactionDetails(
          data.get("transactionid").toString(), "User cancelled the transaction.");

    } else if (null != subReq
        && REQUEST_TYPES.DEPOSIT_FUNDS_SUB_REQ.WITHDRAW_TRANSACTION.equals(subReq)) {
      try {
        Transactions transaction = new Transactions();
        transaction = (Transactions) (data.get("transaction"));
        String thirdPartyTID = new ObjectId().toString();
        int result = Services.getInstance().getFundsDepositService().withdrawDeposit(transaction,
            thirdPartyTID);
