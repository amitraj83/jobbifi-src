...
import com.interview.framework.DISPUTE;
...
public class DisputeHandler extends RequestHandler {
  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    try {
      ...
      if ((SUB_REQ.equals(REQUEST_TYPES.CREATE_DISPUTE))
        ...
        dispute.setWith(interview.getInterviewee().equals(dispute.getCreatedBy()) ? interview
            .getInterviewer() : interview.getInterviewee());
        ...
        if (_id != null) {
          ...
          if (_did != null) {
              ...
              res.put( "disputewith",
                  interview.getInterviewee().equals(dispute.getCreatedBy()) ? interview
                      .getInterviewer() : interview.getInterviewee());
              res.put("amount", dispute.getAmount());
              Map<String, Object> model = new HashMap<String, Object>(); 
              model.put("amount", dispute.getAmount());
              model.put("message", dMessage.getMessage());
              model.put("disputeby", dMessage.getMessageBy());
              model.put("interview_title",interview.getTitle());
              model.put("disputewith",dispute.getWith());
              
          	Services.getInstance().getEmailService().sendMail(DataStoreRegistry.getInstance().getInterviewerDataStore().getUserEmail(dispute.getWith()), 
    	  	   			myProps.getProperty("mail.disputecreated.subject"), "disputecreated.ftl",model);
           ...
           /* int status =
                InformThroughEmail(interview.getInterviewee(), interview.getTitle(), dispute,
                    dMessage);
            int status2 =
                InformThroughEmail(interview.getInterviewer(), interview.getTitle(), dispute,
                    dMessage);*/
        ...
        res.put("title", DataStoreRegistry.getInstance().getInterviewDataStore().getInterview(dispute.getIID()).getTitle()); 
        ...
        if (dispute.getStatus().equals(DATASTORES.DISPUTE.STATUS_TYPE.CLOSED)) {
          if (user.equals(dispute.getCreatedBy()))
            ...
            res.put("disputeResult", dispute.getResult()
                .equals(DATASTORES.DISPUTE.RESULT_TYPE.LOST) ? DATASTORES.DISPUTE.RESULT_TYPE.WIN
                : DATASTORES.DISPUTE.RESULT_TYPE.LOST);
        } else
          res.put("disputeResult", dispute.getResult());
        ...
        int status =
            Services
                .getInstance()
                .getReleaseFundsService()
                .releaseFundsInDispute(new ObjectId(interviewToComplete.getId()),
                    interviewToComplete.getEb());
        if (status == RETURN_VALUES.RELEASE_FUNDS_SUCCESS) {
          // interviewee has lost
          String result = "";
          ...
          DataStoreRegistry.getInstance().getDisputeStore()
               .closeDispute(new ObjectId(disputeId), result,closedBy);
    	  ...
    	  if(list != null){
	    	  ...
	    	  while(it.hasNext()){
	    		  ...
	    		  dispute.setTitle(DataStoreRegistry.getInstance().getInterviewDataStore().getInterview(dispute.getIID()).getTitle());  		
	    	  }
	      }
          res.put("disputeList", list);
      }else if(SUB_REQ.equals(REQUEST_TYPES.CLOSED_DISPUTE_LIST)){
    	  String user = data.get(USER.USERNAME).toString();
    	  List<Dispute> list= DataStoreRegistry.getInstance().getDisputeStore().getClosedDisputelist(user);
    	  if(list != null){
	    	  Iterator it = list.iterator();
	    	  while(it.hasNext()){
	    		  Dispute dispute =(Dispute) it.next();	    		  	  
        ...
        int status =
            Services
                .getInstance()
                .getReleaseFundsService()
                .revertFundsInDispute(new ObjectId(interviewToComplete.getId()),
                    interviewToComplete.getEb());
        if (status == RETURN_VALUES.REVERT_FUNDS_SUCCESS) {
          ...
          DataStoreRegistry
              ...
              .updateInterviewStatus(new ObjectId(interviewToComplete.getId()),
  ...
  private int InformThroughEmail(String userToSend, String interviewTitle, Dispute dispute,
      DisputeMessage dMessage) throws RemoteException {
    /*
     * Map<String, Object> userMap = DataStoreRegistry.getInstance() .getInterviewerDataStore()
     * .getUserInfo(userToSend);
     */
    String userEmail =
        DataStoreRegistry.getInstance().getInterviewerDataStore().getUserEmail(userToSend);
    // String intervieweeUserName = userMap.get(USER.USERNAME).toString();
    String emailBody =
        DISPUTE.CREATE_DISPUTE_EMAIL_BODY.replaceAll("$user$", userToSend)
            .replaceAll("$ititle$", interviewTitle)
            .replaceAll("$disputer$", dispute.getCreatedBy())
            .replaceAll("$message$", dMessage.getMessage()).replaceAll("$did$", dispute.getId());
    return Services
        .getInstance()
        .getEmailService()
        .sendEmail("support@" + myProps.getProperty("emailhost"), userEmail,
            DISPUTE.CRETE_DISPUTE_EMAIL_SUBJECT,
            dMessage.getFid() != null ? dMessage.getFid() : "", emailBody);
  }
