  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    try {
      ...
      if(SUB_REQ !=null && SUB_REQ.equals(REQUEST_TYPES.GET_ALL_BIDS)){
    	  ...
    	  List<Bid> posted = DataStoreRegistry.getInstance().getBidStore().getBidsIPosted(username,pageNumber);
    	  if(posted != null){    		 
    		for(Bid bid:posted){
    			// TODO : change
    			Interview interview = DataStoreRegistry.getInstance().getInterviewDataStore().getInterview(bid.getIid());
    		...
    		resMap.put("ALL_BID_PLACED_count", DataStoreRegistry.getInstance().getBidStore().getBidsIPostedCount(username));
    	  }
    	  ...
    	  int pageNumber = new Integer(data.get("pagenum").toString());
    	  List<Object> listOfBids = new ArrayList<Object>();
    	  List<Interview> listofInterview =DataStoreRegistry.getInstance().getInterviewDataStore().getAwardedInterview(username,pageNumber);
    	  for(Interview interview:listofInterview){
    		  Bid bid=DataStoreRegistry.getInstance().getBidStore().getBidByBidderAndInterview(username,interview.getId());
    		  interview.setPrice(bid.getPrice());
    		  interview.setDt(new Date((Long) bid.getDate()).getTime());
    		  listOfBids.add(interview);
    	  }
    		resMap.put("AWARDED_BID", listOfBids);
    		resMap.put("AWARDED_BID_count", DataStoreRegistry.getInstance().getInterviewDataStore().getTotalAwardedInterviewCount(username));
    	  
      } else {
    	  List<Bid> posted = DataStoreRegistry.getInstance().getBidStore().getBidsIPosted(username);
	      if(posted != null){
	    	  for(Bid bid : posted) {	    	 
	    		bid.setReputation(DataStoreRegistry.getInstance().getRatingStore() .getReviewsCount(username)) ;
	    	  }      				
    	  }
	      resMap.put(VARIABLES.BIDS_POSTED, (posted));
	
	      List<Map<String, Object>> listOfBids = new ArrayList<Map<String, Object>>();
	      List<Interview> listOfMyPostedInterviews =
	      ...
	      for (Interview interview : listOfMyPostedInterviews) {
	        ...
	        for (Bid bid : received) {
	          ...
	          bidMap.put(VARIABLES.BIDDERS_RATING, DataStoreRegistry.getInstance().getRatingStore()
	              .getAvgRating(bid.getBidder()));
	          ...
	          bidMap.put("bidderreviewcount", DataStoreRegistry.getInstance().getRatingStore()
	              .getReviewsCount(bid.getBidder()));
