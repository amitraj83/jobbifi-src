...
public class JobApplicationHandler extends RequestHandler {
  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    if (null != SUB_REQ && REQUEST_TYPES.JOB_APPLICATION_SUB_REQ.SAVE_APPLICATION.equals(SUB_REQ)) {
    ...
    } else if (null != SUB_REQ
        && REQUEST_TYPES.JOB_APPLICATION_SUB_REQ.GET_APPLICATION_BY_JOB_AND_USER.equals(SUB_REQ)) {
      ...
      try {
        List<JobApplication> jobApplications = DataStoreRegistry.getInstance()
            .getJobApplicationStore().getJobApplicationsByJobIdAndUserId(jobId, userId);
        if (jobApplications.size() > 0) {
          UploadedFile uploadedFile = DataStoreRegistry.getInstance().getUploadedFileDataStore()
              .getUploadedFile(new ObjectId(jobApplications.get(0).getJobId()));
        ...
        && REQUEST_TYPES.JOB_APPLICATION_SUB_REQ.UPDATE_APPLICATION_STATUS.equals(SUB_REQ)) {
      String id = (String) data.get(DATASTORES.JOB_APPLICATION.ID);
      String status = (String) data.get(DATASTORES.JOB_APPLICATION.STATUS);
      try {
        boolean result =
            DataStoreRegistry.getInstance().getJobApplicationStore().updateStatus(id, status);
        if (result) {
          resMap.put("status", 1);
        } else {
          resMap.put("status", -1);
