    	  ...
    	  if(null != subReq && subReq.toString().equals(REQUEST_TYPES.RATING_SUB_REQ.RATE_USER)) {    	
		      ...
		      if (rid != null) {
		        ...
		        Services.getInstance().getNotificationService()
		            .processNotification(rating, VARIABLES.NOTIFICATION.TYPE.RATING_NOTIFICATION);
		
		        resMap.put("status", RETURN_VALUES.getResponseMessage(RETURN_VALUES.RATING_SAVED_SUCCESS));
		        resMap.put("code", RETURN_VALUES.RATING_SAVED_SUCCESS);
		      } else {
		        resMap.put("status", RETURN_VALUES.getResponseMessage(RETURN_VALUES.RATING_SAVED_FAILURE));
		        resMap.put("code", RETURN_VALUES.RATING_SAVED_FAILURE);
		      }
		      
    	  } else if(null != subReq && subReq.toString().equals(REQUEST_TYPES.RATING_SUB_REQ.GET_USER_RATING)){
