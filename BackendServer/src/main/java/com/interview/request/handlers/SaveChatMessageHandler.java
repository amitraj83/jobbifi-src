	...
	if(null != subRequest && REQUEST_TYPES.SAVE_CHAT_MESSAGE.equals(subRequest)){	
	    try {
		 ...
		 try {
		      ...
		      try {
		    	  Map<String, Object> responseMap = DataStoreRegistry.getInstance().getInterviewerDataStore().getUserInfo(cm.getTo());
		      ...
		      if (result == 1) {
		    	  ...
		    	  String toUserEmail = DataStoreRegistry.getInstance().getInterviewerDataStore().getUserEmail(cm.getTo());
			      Services.getInstance().getEmailService().sendEmail(myProps.getProperty("mail.from.info"),
			    		  toUserEmail , "Message", null, "Dear " + cm.getTo() + ", \n\n You are contacted by "+cm.getFrom()+". Message sent : \n\n "
			    		  + cm.getMessage() + " \n\n Regards, \n Team Mock Interview");
			      
			      String fromUserEmail = DataStoreRegistry.getInstance().getInterviewerDataStore().getUserEmail(cm.getFrom());
			      Services.getInstance().getEmailService().sendEmail(myProps.getProperty("mail.from.info"),
			    		  fromUserEmail, "Message sent confirmation", null, "Dear " + cm.getFrom() + ", \n\n Your message sent to "+cm.getTo()+" successfully. "
			    		  + " \n\n Regards, \n Team Mock Interview");
