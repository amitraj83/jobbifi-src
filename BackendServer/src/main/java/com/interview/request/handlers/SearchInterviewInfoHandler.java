  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    if (null != subReq
		...
		try {			
			...
			Interview userinfo = DataStoreRegistry.getInstance().getInterviewDataStore().getInterview(id);
		...
		String username = (data.get(USER.USERNAME) != null) ? data.get(USER.USERNAME).toString() : null;
	    ...
	    try {
	      ...
	      if(null != username) {
	    	  List<Bid> bids = DataStoreRegistry.getInstance().getBidStore().getBidsForInterviewsByUser(username, iids);
	    	  for(Bid bid: bids){
