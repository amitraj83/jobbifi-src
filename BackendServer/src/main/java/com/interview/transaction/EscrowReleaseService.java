...
public class EscrowReleaseService {
	...
	public int escrowRelease(Interview interview, double amount ){
		...
		try{
			if(eb >= amount){
				...
				.getInterviewerDataStore().getUserInfo(interview.getInterviewee());
				double balance = new Double( userInfo.get(USER.BALANCE).toString());
				
				interviewerPrevBalance = new Double( DataStoreRegistry.getInstance().getInterviewerDataStore()
				...
				double newBalance = DataStoreRegistry.getInstance()
										.getInterviewerDataStore()
				...
				if(diff == net)
				{
					...
					cAccountId = DataStoreRegistry.getInstance()
					.getCompanyAccountStore().insertTransaction(cAccount);
					
					
					stage = 4;
					
					transactionStage4  = Services.getInstance().getTransactionHistoryService()
					.logTransaction(new Date().getTime(), 
							DATASTORES.TRANSACTION.TTYPE.CREDIT, 
							"ESCROW", interview.getInterviewer(), 
							DATASTORES.TRANSACTION.TSTATUS.DONE, 
							"Interview Payment", amount, 0,amount, balance);
					...
					userTransactionStage5.setPurpose(DATASTORES.USER_TRANSACTION.TRANSACTION_PURPOSE_TYPE.ESCROW_RELEASE);
					...
					String ut_id = DataStoreRegistry.getInstance().getUserTransactionStore().save(userTransactionStage5);
					userTransactionStage5.set_id(ut_id);
					
					stage = 6;

					transactionStage6 = Services.getInstance().getTransactionHistoryService()
					.logTransaction(new Date().getTime(), 
							DATASTORES.TRANSACTION.TTYPE.DEBIT, 
							interview.getInterviewee(), interview.getInterviewer(),  
							DATASTORES.TRANSACTION.TSTATUS.DONE, 
							"Interview Payment", amount, fee,net, newBalance);
					...
					userTransactionStage7.setPurpose(DATASTORES.USER_TRANSACTION.TRANSACTION_PURPOSE_TYPE.ACCOUNT_DEPOSIT);
					...
					/*if(interviewPrevStatus == INTERVIEW_STATUS.DISPUTE)
						DataStoreRegistry.getInstance().getInterviewDataStore()
						.updateInterviewStatus(new ObjectId( interview.getId()), INTERVIEW_STATUS.FULLY_PAID);
					else
						DataStoreRegistry.getInstance().getInterviewDataStore()
								.updateInterviewStatus(new ObjectId( interview.getId()), INTERVIEW_STATUS.FULLY_PAID);
					*/
	...
	private void revertState8(Interview interview, int interviewPrevStatus) {
		try {
			DataStoreRegistry.getInstance().getInterviewDataStore().updateInterviewStatus(new ObjectId(interview.getId()), interviewPrevStatus);
	...
	private void revertStage5(UserTransaction userTransactionStage5)
		...
		if(userTransactionStage5 != null && userTransactionStage5.get_id() != null){
			boolean deleted = DataStoreRegistry.getInstance().getUserTransactionStore().deleteUserTransaction(userTransactionStage5.get_id());
			if(!deleted)
	...
	private void revertStage4(Transaction transactionStage4)
		...
		if(transactionStage4 != null){
			boolean deleted =DataStoreRegistry.getInstance().getTransactionStore().deleteTransaction(transactionStage4.getId());
			if(!deleted)
	...
	private void revertStage3(String cAccountId) {
		if(cAccountId != null){
			try {
				boolean deleted = DataStoreRegistry.getInstance().getCompanyAccountStore().deleteTransaction(cAccountId);
				if(!deleted){
	...
	private void revertStage2(Interview interview, Double interviewerPrevBalance) {
		if(interviewerPrevBalance != null){
			try {
				DataStoreRegistry.getInstance()
				.getInterviewerDataStore().setBalance(interview.getInterviewer(), interviewerPrevBalance);
			} catch (RemoteException e) {
				e.printStackTrace();
			}
		}
	...
	private void revertStage1(Interview interview, double eb) {
		...
		try {
			DataStoreRegistry.getInstance().getInterviewDataStore().updateInterview(
					new ObjectId( interview.getId()), changes);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
