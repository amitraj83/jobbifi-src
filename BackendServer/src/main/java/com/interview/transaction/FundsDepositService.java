...
public class FundsDepositService {
  ...
  public int deposit(String user, double amount, String thirdPartyId) {
    ...
    try {
      double prev_bal = new Double(DataStoreRegistry.getInstance().getInterviewerDataStore()
          .getUserInfo(user).get(USER.BALANCE).toString());
      DataStoreRegistry.getInstance().getInterviewerDataStore().updateBalance(user, amount,
          VARIABLES.ADD);
      stage = 1;
      ...
      if ((bal - prev_bal) == amount) {
        ...
        Transaction transaction = Services.getInstance().getTransactionHistoryService()
            .logTransaction(new Date().getTime(), DATASTORES.TRANSACTION.TTYPE.DEBIT, user, user,
                DATASTORES.TRANSACTION.TSTATUS.DONE, "Funds deposited ", amount, 0, amount, bal);
        stage = 3;
        ...
        stage = 5;
        Services.getInstance().getNotificationService().processNotification(transaction,
      ...
      if (stage == 2 || stage == 3 || stage == 4) {
        ...
        ticket.setAgent("");
        ticket.setCreatedBy(user);
        String description = "Transaction history missing :\r\n" + "DEBIT_OR_CREDIT:"
  ...
  public String logtransaction(Transactions transactions, String thirdPartyId) {
    ...
    try {
      double prev_bal = new Double(DataStoreRegistry.getInstance().getInterviewerDataStore()
          .getUserInfo(transactions.getOwner()).get(USER.BALANCE).toString());
      Transaction transaction =
          Services.getInstance().getTransactionHistoryService().logTransaction(new Date().getTime(),
              DATASTORES.TRANSACTION.TTYPE.DEBIT, transactions.getOwner(), transactions.getOwner(),
              DATASTORES.TRANSACTION.TSTATUS.PENDING, "Paypal Initiated ", transactions.getAmount(),
              0, transactions.getAmount(), prev_bal);
  ...
  public int withdrawDeposit(Transactions transactions, String thirdPartyId) {
    ...
    try {
      ...
      if (prev_bal < amount)
        ...
        Transaction transaction = Services.getInstance().getTransactionHistoryService()
            .logTransaction(new Date().getTime(), DATASTORES.TRANSACTION.TTYPE.DEBIT, user, user,
                DATASTORES.TRANSACTION.TSTATUS.PENDING, "Funds Withdrow from your account ", amount,
                0, amount, balance);
        UserTransaction userTransaction = new UserTransaction();
        ...
        DataStoreRegistry.getInstance().getUserTransactionStore().save(userTransaction);
        String toUserEmail =
            DataStoreRegistry.getInstance().getInterviewerDataStore().getUserEmail(user);
        Map<AttributeType, String> model = new HashMap<AttributeType, String>();
        model.put(AttributeType.USER_NAME, user);
        model.put(AttributeType.FUND_AMOUNT, String.valueOf(amount));
        Services.getInstance().getEmailService().sendMail(Mailer.EmailType.NEW_FUND_WITHDRAWAL,
            model, toUserEmail);
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public int funddeposit(String transactionid, String thirdPartyId) {
    ...
    int stage = 0;
    try {
      Transaction transaction = (DataStoreRegistry.getInstance().getTransactionStore()
          .getTransaction(new ObjectId(transactionid)));
      ...
      if ((bal - prev_bal) == amount) {
        stage = 2;
        transaction = Services.getInstance().getTransactionHistoryService().updateTransaction(
            transactionid, new Date().getTime(), DATASTORES.TRANSACTION.TTYPE.CREDIT, user, user,
            DATASTORES.TRANSACTION.TSTATUS.DONE, "Funds deposited from paypay to your account",
            amount, 0, amount, bal);
        stage = 3;
        UserTransaction userTransaction = new UserTransaction();
        userTransaction.setArtifact(DATASTORES.USER_TRANSACTION.ARTIFACT_TYPE.THIRD_PARTY_DEPOSIT);
  ...
  public void updateTransactionDetails(String transactionid, String details) {
    try {
      DataStoreRegistry.getInstance().getTransactionStore().updateTransactionDetails(transactionid,
          details);
    } catch (Exception e) {
