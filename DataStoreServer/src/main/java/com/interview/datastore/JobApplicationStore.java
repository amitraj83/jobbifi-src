...
public class JobApplicationStore extends UnicastRemoteObject implements IJobApplicationStore {
  ...
  public JobApplication getJobApplication(String _id) throws RemoteException {
    ...
    if (row != null) {
      ...
      jobApplication.setCoverLetter(
          MongoDataHelper.getStringValue(row, DATASTORES.JOB_APPLICATION.COVER_LETTER));
      jobApplication
          .setCvFileId(MongoDataHelper.getStringValue(row, DATASTORES.JOB_APPLICATION.CV_FILE_ID));
      jobApplication
          .setJobId(MongoDataHelper.getStringValue(row, DATASTORES.JOB_APPLICATION.JOB_ID));
      jobApplication.setApplicantId(
          MongoDataHelper.getStringValue(row, DATASTORES.JOB_APPLICATION.APPLICANT_ID));
      jobApplication.setDt(MongoDataHelper.getLong(row, DATASTORES.JOB_APPLICATION.DATE));
      jobApplication
          .setStatus(MongoDataHelper.getStringValue(row, DATASTORES.JOB_APPLICATION.STATUS));
    }
  ...
  public boolean updateStatus(String id, String status) throws RemoteException {
    ...
    DBObject updateDoc =
        new BasicDBObject("$set", new BasicDBObject(DATASTORES.JOB_APPLICATION.STATUS, status));
