...
public class TestDataStore extends UnicastRemoteObject implements ITestDataStore {
	...
	public List<Test> getAllTests() throws RemoteException {
		 DBCollection collection = Services.getInstance().getBaseDataStore().db
				 .getCollection(DATASTORES.TEST.DBCollection);
	    List<Test> list = new ArrayList<Test>();
	    DBObject query = new BasicDBObject();	   
	    DBCursor cursor = collection.find(query)
	    		.sort(new BasicDBObject(DATASTORES.TEST.CREATED_DATE, -1));
	    while (cursor.hasNext()) {
	      ...
	      if(row != null) {
		      ...
		      test.setDifficultyLevel(Integer.parseInt(row.get(DATASTORES.TEST.DIFFICULTY_LEVEL).toString()));
