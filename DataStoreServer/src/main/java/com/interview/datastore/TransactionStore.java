...
public class TransactionStore extends UnicastRemoteObject implements ITransactionStore {
  ...
  public Transaction getTransaction(ObjectId tid) throws RemoteException {
    ...
    transaction.setBalance(new Double(row.get(DATASTORES.TRANSACTION.BALANCE).toString()));
    transaction.setDetails(row.get(DATASTORES.TRANSACTION.DETAILS).toString());
    transaction.setFee(new Double(row.get(DATASTORES.TRANSACTION.FEE).toString()));
    transaction.setGross(new Double(row.get(DATASTORES.TRANSACTION.GROSS).toString()));
    transaction.setId(row.get("_id").toString());
    transaction.setNetamount(new Double(row.get(DATASTORES.TRANSACTION.NETAMOUNT).toString()));
    transaction.setOtherParty(row.get(DATASTORES.TRANSACTION.OTHERPARTY).toString());
    transaction.setOwner(row.get(DATASTORES.TRANSACTION.OWNER).toString());
    transaction.setStatus(row.get(DATASTORES.TRANSACTION.STATUS).toString());
    transaction.setTime(new Long(row.get(DATASTORES.TRANSACTION.TIME).toString()));
  ...
  public List<Transaction> getTransactionsList(String user, long fromDate, long toDate, int pageNum)
    ...
    List<String> tids =
        Services.getInstance().getUserTransactionDataStore().getTransactionIDs(user);
    if (tids.size() > 0) {
      ...
      while (cursor.hasNext()) {
        ...
        Transaction trans =
            Services.getInstance().getJSONUtilityService().convertValue(row, Transaction.class);
  ...
  public int updateTransaction(String transactionid, double balance, String details)
    ...
    DBObject newDoc = new BasicDBObject().append(DATASTORES.TRANSACTION.BALANCE, balance)
