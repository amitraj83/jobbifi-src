...
public class FileServer implements DisposableBean {
	...
	private void startServer(String serviceName, String zkConnectStr) {
		...
		ns.registerHandler(new RequestHandler<Request, Response>() {
			...
			public Response handleRequest(Request request) throws Exception {
				...
				com.interview.request.handlers.RequestHandler handler = Services
						.getInstance()
						.getRequestHandler(requestType.toString());

				String protoMap = request.getData();
				JSONUtilityService jsonService = Services.getInstance()
						.getJSONUtilityService();
				Map<Object, Object> mapObject = jsonService.readValue(protoMap,
						Map.class);
				Map<String, Object> response = handler.handleRequest(mapObject);

				return Response.newBuilder()
						.setData(jsonService.getJSONStringOfMap(response))
						.build();
			}
	...
	private void configCluster(String serviceName, String zkConnectStr) {
		final ClusterClient cc = new ZooKeeperClusterClient(serviceName,
				zkConnectStr, 30000);
	...
	public void destroy() throws Exception {
		...
		final ClusterClient cc = new ZooKeeperClusterClient(
				norbertFileServiceName, norbertZkConnectionString, 30000);
		cc.awaitConnectionUninterruptibly();
