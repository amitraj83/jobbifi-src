...
@Service
public class SaveChatFileHandler extends RequestHandler {

  @Autowired
  private Properties myProps;


  public SaveChatFileHandler() {
    addHandler(this, REQUEST_TYPES.FILESERVER_SAVE_CHAT_FILE);
  }
  ...
  public Map<String, Object> handleRequest(Map<Object, Object> data) {
    ...
    int status = Services.getInstance().getFileUtilities().copyFile(username, is,
        myProps.getProperty("chatDocDir"), fileNameWithExtension);
    if (status == 1) {
      ...
      file.setPath_on_Disk(username + File.separator + _id + "." + extension);
      file.setSize(new Long(data.get(DATASTORES.UPLOAD_FILE.SIZE).toString()));
      file.setThumbnailURL("");
      file.setTime(new Date().getTime());
      file.setURL("aauth/chatfiles/chatfiledownload.do?fileid=" + _id);
