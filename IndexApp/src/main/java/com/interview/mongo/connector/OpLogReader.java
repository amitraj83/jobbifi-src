...
public class OpLogReader implements Runnable {
  ...
  protected OplogLine parseLogLine(BSONTimestamp ts, DBObject x) {
    ...
    BasicDBObject idObject = ((BasicDBObject) x.get("o2"));
    if (idObject != null && idObject.containsKey("_id")) {
      String _id = idObject.getString("_id");
      data.put("_id", _id);
      log.debug("Checking Id for o2 : " + _id);
    }



    OplogLine line = new OplogLine(operation, timestamp, nameSpace, data);
    return line;
  }
}
