...
public class InterviewConversion {
	...
	public static SolrInterview getInterviewForUpdate(
			BasicDBObject mongoInterview) {

		SolrInterview interview = getInterviewPojo(
				mongoInterview.getString("_id"),
				(BasicDBObject) mongoInterview.get("$set"));
		return interview;
	}

	public static SolrInterview getInterviewForInsert(
			BasicDBObject mongoInterview) {

		SolrInterview interview = getInterviewPojo(
				mongoInterview.getString("_id"), mongoInterview);
		return interview;
	}
	...
	private static SolrInterview getInterviewPojo(String _id, DBObject row) {
		...
		if (row.get(DATASTORES.INTERVIEW.DESCRIPTION) != null)
			solrInterview.setDescription(row.get(
					DATASTORES.INTERVIEW.DESCRIPTION).toString());
		solrInterview.setDoctype(DOCTYPE.INTERVIEW);
		if (row.get(DATASTORES.INTERVIEW.DATE) != null)
			solrInterview.setDt(new Long(row.get(DATASTORES.INTERVIEW.DATE)
					.toString()));
		if (row.get(DATASTORES.INTERVIEW.INTERVIEWEE) != null)
			solrInterview.setInterviewee(row.get(
					DATASTORES.INTERVIEW.INTERVIEWEE).toString());
		
		if (row.get(DATASTORES.INTERVIEW.SKILLS) != null) {
			BasicDBList list = (BasicDBList) row
					.get(DATASTORES.INTERVIEW.SKILLS);
		...
		if (row.get(DATASTORES.INTERVIEW.TITLE) != null)
			solrInterview.setTitle(row.get(DATASTORES.INTERVIEW.TITLE)
					.toString());
