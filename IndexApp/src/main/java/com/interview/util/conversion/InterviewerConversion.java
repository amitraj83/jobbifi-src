...
public class InterviewerConversion {
	...
	public static SolrInterviewer getInterviewerForUpdate(BasicDBObject data) {
		...
		SolrInterviewer interviewer = getInterviewerPojo(data.getString("_id"),
				(BasicDBObject) data.get("$set"));
		return interviewer;
	}
	...
	private static SolrInterviewer getInterviewerPojo(String _id, DBObject data) {
		...
		if (data.get(USER.EDUCATIONS) != null) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				DBObject query = new BasicDBObject("_id", new ObjectId(
						educationId));
				DBObject obj = MongoDBService.getInstance()
						.getEducationCollection().findOne(query);

				fieldOfStudy[i] = obj.get(DATASTORES.EDUCATION.FIELDOFSTUDY)
						.toString();
				schools[i] = obj.get(DATASTORES.EDUCATION.SCHOOLNAME)
						.toString();
		...
		if (data.get(USER.POSITIONS) != null) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				DBObject obj = MongoDBService.getInstance()
						.getPositionCollection().findOne(query);
		...
		if (data.get("skillids") != null) {
			...
			for (int i = 0; i < SkillIDs.size(); i++) {
				...
				DBObject obj = MongoDBService.getInstance()
						.getSkillsCollection().findOne(query);
