...
public class JobConversion {
  ...
  public static SolrJob getJobForUpdate(BasicDBObject mongoJob) {
    ...
    SolrJob solrJob = getJobPojo(mongoJob.getString("_id"), (BasicDBObject) mongoJob.get("$set"));
    return solrJob;
  }
  ...
  private static SolrJob getJobPojo(String _id, DBObject row) {
    ...
    if (row.get(DATASTORES.JOB.DESCRIPTION) != null)
      solrJob.setDescription(row.get(DATASTORES.JOB.DESCRIPTION).toString());
    ...
    if (row.get(DATASTORES.JOB.INTERVIEWER) != null)
      solrJob.setInterviewer(row.get(DATASTORES.JOB.INTERVIEWER).toString());
    ...
    if (row.get(DATASTORES.JOB.COMPANY_NAME) != null) {
      solrJob.setCompanyName(row.get(DATASTORES.JOB.COMPANY_NAME).toString());
    }

    if (row.get(DATASTORES.JOB.LOCATION) != null) {
      solrJob.setLocation(row.get(DATASTORES.JOB.LOCATION).toString());
    }
