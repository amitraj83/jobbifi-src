...
public class DepositFundsController extends BaseController {
    ...
    public ModelAndView depositFunds(ModelMap model, HttpServletRequest req, HttpServletResponse res) {
      ...
      try {
			...
			String accessToken = new OAuthTokenCredential(myProps.getProperty("paypalaccesskey"), myProps.getProperty("paypalsecurekey"), sdkConfig).getAccessToken();
			...
			for(int i=0; i<createdPayment.getLinks().size();i++){
				if(createdPayment.getLinks()!=null && createdPayment.getLinks().get(i).getRel().equals("approval_url"))
