...
<script type="text/javascript">
		...
		$
				...
				.done(
						function(msg) {
										...
										.each(
												...
												function(i) {
													...
													for (var j = 0; j < json[i].additional.positions.length; j++) {
														...
														positions
																.push({
																	position : json[i].additional.positions[j].title,
																	company : json[i].additional.positions[j].companyName,
																	icon : icon
																});
													}
													var educations = [];
													for (var j = 0; j < json[i].additional.educations.length; j++) {
														...
														educations
																.push({
																	education : json[i].additional.educations[j].degree,
																	field : json[i].additional.educations[j].fieldOfStudy,
																	school : json[i].additional.educations[j].schoolname,
																	icon : icon
																});
													}
													var searchItem = '<div class="row">'
															+ '<div class="col-md-3">'
							...
							var html = getPaginationHTMLForAdvisor(
									totalRecords, currentPage, searchKey,
									"INTERVIEWER");
	...
	function getPaginationHTMLForAdvisor(totalRecords, currentPage, searchKey,
			searchFor) {
		...
		if (totalPages > 1) {
			...
			for (var i = 0; i < totalPages; i++) {
				if (currentPage == (i + 1)) {
					...
					html = html
							+ "<li class=''><a href='javascript:searchAdvisors(\""
							+ searchKey + "\"," + (i + 1) + ")'>" + (i + 1)
							+ "</a></li>";
				}
			}
