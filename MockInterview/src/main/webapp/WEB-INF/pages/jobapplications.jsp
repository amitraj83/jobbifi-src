...
function showJobsOffered() {
        ...
        success: function (res) {
			...
			if (null != jobs && jobs.length > 0) {
				for (var i = 0; i < jobs.length; i++) {
					var jobItem = '<div class="row job-applications job">'
							+ '	<div class="col-md-10">'
							+ '		<div class="row job-title">{{ title }}</div>'
							+ '		<div class="row job-company">{{ company }}</div>'
							+ '		<div class="row job-location">{{ location }}</div>'
							+ '	</div>'
							+ '	<div class="col-md-2">'
							+ '		<a href="/jobdetail.do?jid={{ id }}" class="btn btn-success btn-lg job-view-btn">View Job</a>	'
							+ '	</div>'
							+ '</div>';
					...
					jobItem = Mustache.to_html(jobItem, templateData);
					jobsHtml += jobItem;
				}
			} else {
	...
	function getUserProfileLink(user) {
		return "<a target='_blank' href='" + BASE_URL + "userprofile.do?name="
				+ user + "'>" + user + "</a>";
	}
	...
	function getApplicantHtml(job) {
		...
		var html = '<div class="table-responsive">'
				...
				+ '<th>Applicant</th>' + '<th>Cover Letter</th>'
				+ '<th>Resume</th>' + '<th>Date</th>'
				+ '<th style="width:15%">Status</th>' + '</tr>' + '</thead>';
			...
			for (var i = 0; i < applications.length; i++) {
				html += "<tr>"
    				...
    				applications[i].applicantId + "' ></div>"
						+ "<div class='col-md-8'>"
						+ getUserProfileLink(applications[i].applicantId)
						+ "<br/>"
						+ "<input type='number' class='user-rating' value='"+applications[i].rating+"'/>"
						+ "<div><span id='reviewcount'> "
						+ applications[i].reviewCount + "</span> Reviews</div>"
						+ "</div>" + "</div>" + "</td>" + "<td>"
						+ applications[i].coverLetter + "</td>";
				if (applications[i].cvFileId != "") {
					html += "<td><a target='_blank' href='"+BASE_URL+applications[i].uploadedFile.url+"'>"
							+ applications[i].uploadedFile.originalFileName
							+ "</a></td>";
				} else {
					html += "<td>NA</td>";
				}
				...
				html += "<td>"
						+ prettyDate(new Date(applications[i].dt))
						+ "</td>"
						+ "<td>"
						+ getStatusSelectBox(applications[i].status,
								applications[i].id) + "</td>" + "</tr>";
			}
	...
	function getStatusSelectBox(status, applicationId) {
		...
		var selectHtml = "<select class='form-control' data-appid='"
				+ applicationId + "' onchange='changeApplicationStatus(this)'>";
		for (var i = 0; i < statuses.length; i++) {
			if (statuses[i] == status) {
				selectHtml += "<option selected='selected' value='"+statuses[i]+"'>"
						+ statuses[i] + "</option>";
			} else {
				selectHtml += "<option value='"+statuses[i]+"'>" + statuses[i]
						+ "</option>";
			}
