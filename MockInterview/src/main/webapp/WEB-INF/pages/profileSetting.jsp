...
<body>
	...
	<script type="text/javascript"> var username = '<sec:authentication property="principal.username" />';
			    ...
			    maxChunkSize: 20000000,
			        ...
			        if (null != jsonResponse) {
			            if (jsonResponse.error == "1") {
			                ...
			                $("#update_img").attr({
			                    src: BASE_URL + jsonResponse.path
			                });
			            }
			        }
			    },
					...
					.ajax({
						...
						success : function(res) {
							...
							for (var i = 0; i < allskills.length; i++) {
								editSkillSet(allskills[i].skill,
							...
							for (var i = 0; i < allpositions.length; i++) {
								...
								var screen = positionHTML.split("$title$")
										.join(allpositions[i].title).split(
												"$cn$").join(
												allpositions[i].companyName)
										.split("$syui$").join(sel).split(
												"$eyui$").join(sel2).split(
												"$desc$").join(
							...
							for (var i = 0; i < alleducations.length; i++) {
								...
								var screen = educationHTML.split("$degree$")
										.join(alleducations[i].degree).split(
												"$major$").join(
