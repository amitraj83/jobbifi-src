...
Date.fromString = (function () {
  ...
  var create_absolute = function (obj) {
	...
	var date = new Date();
	if(!defaults.opts.convertUTC)
		date.setTime(Date.parse(year + '/' + month + '/' + day + ' ' + time));
	else
		date = new Date(Date.UTC(year,month-1,day,parts[0],parts[1],parts[2]));
    return date;
  };
  ...
  return function (fuzz, opts) {
    ...
    if (opts && opts.convertUTC != undefined) defaults.opts.convertUTC = opts.convertUTC;
