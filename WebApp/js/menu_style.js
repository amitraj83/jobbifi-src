var ddaccordion = {
    ...
    ajaxloadcontent: function ($targetHeader, $targetContent, config, callback) {
        ...
        if (ajaxinfo.status == "none") { /*ajax data hasn't been fetched yet*/
            ...
            /*set ajax status to "loading"*/
            $.ajax({
                url: ajaxinfo.url, /*path to external menu file*/ error: function (ajaxrequest) {
    ...
    expandit: function ($targetHeader, $targetContent, config, useractivated, directclick, skipanimation) {
        ...
        if (ajaxinfo) { /*if this content should be fetched via Ajax*/
            if (ajaxinfo.status == "none" || ajaxinfo.status == "loading") this.ajaxloadcontent($targetHeader, $targetContent, config, function () {
        ...
        $targetContent.slideDown(skipanimation ? 0 : config.animatespeed, function () {
            ...
            if (config.postreveal == "gotourl" && directclick) { /*if revealtype is "Go to Header URL upon click", and this is a direct click on the header*/
                ...
                if (targetLink) /*if this header is a link*/ setTimeout(function () {
    ...
    transformHeader: function ($targetHeader, config, state) {
        $targetHeader.addClass((state == "expand") ? config.cssclass.expand : config.cssclass.collapse)/*alternate btw "expand" and "collapse" CSS classes*/.removeClass((state == "expand") ? config.cssclass.collapse : config.cssclass.expand)
    ...
    getCookie: function (Name) {
        ...
        if (document.cookie.match(re)) /*if cookie found*/ return document.cookie.match(re)[0].split("=")[1]
        /*return its value*/
        return null
    },
    ...
    init: function (config) {
        ...
        jQuery(document).ready(function ($) {
            ...
            if (config.revealtype == "clickgo") {
                ...
                /*remember added action*/
                config.revealtype = "click"
                /*overwrite revealtype to "click" keyword*/
            }
            if (typeof config.togglehtml == "undefined") config.htmlsetting = {location: "none"} else config.htmlsetting = {
            ...
            /*attach custom "onopenclose" event handler*/
            var lastexpanded = {}
            /*object to hold reference to last expanded header and content (jquery objects)*/
            var expandedindices = ddaccordion.urlparamselect(config.headerclass) || ((config.persiststate && persistedheaders != null) ? persistedheaders : config.defaultexpanded)
            if (typeof expandedindices == 'string') /*test for string value (exception is config.defaultexpanded, which is an array)*/ expandedindices = expandedindices.replace(/c/ig, '').split(',')
            /*transform string value to an array (ie: "c1,c2,c3" becomes [1,2,3]*/
            if (expandedindices.length == 1 && expandedindices[0] == "-1") /*check for expandedindices value of [-1], indicating persistence is on and no content expanded*/ expandedindices = []
            if (config["collapseprev"] && expandedindices.length > 1) /*only allow one content open?*/ expandedindices = [expandedindices.pop()]
            /*return last array element as an array (for sake of jQuery.inArray())*/
            if (config["onemustopen"] && expandedindices.length == 0) /*if at least one content should be open at all times and none are, open 1st header*/ expandedindices = [0]
