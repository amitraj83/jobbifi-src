...
$("#searchMockResult").on("click",'.candidates-item .read-more, .candidates-item .toggle', function(event){	
	event.preventDefault();
	var $item = $(event.target).closest('.candidates-item'),
		$content = $item.find('.content'),
		$toggle = $item.find('.top-btns .toggle'),
		$descLess = $item.find('.descLess');
...
$("#searchJobResult").on("click",'.jobs-item .read-more, .jobs-item .toggle', function(event){	
	event.preventDefault();
	var $item = $(event.target).closest('.jobs-item'),
		$content = $item.find('.content'),
		$toggle = $item.find('.top-btns .toggle'),
		$descLess = $item.find('.descLess');
...
function getDescriptionLess(description, length){
	var len = length || 180;
	var descriptionLess = description.substring(0,len);
	if(description.length > len){
		descriptionLess += "...";
	}
	return descriptionLess;
}
